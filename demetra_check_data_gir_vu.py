...
def fix_files_girvu(data_folder:str, end_folder:str):
    """
            ...
            for dirpath, dirnames, filenames in os.walk(data_folder):
                for file in filenames:
                    if not file.startswith('~$') and (file.endswith('.xlsx') or file.endswith('.xlsm')):
                        ...
                        if 'Код профессии, специальности, по которой проводится обучения (для программ СПО' in df.columns:
                            df.rename(columns={'Код профессии, специальности, по которой проводится обучения (для программ СПО':
                                               'Код профессии, специальности, по которой проводится обучения (для программ СПО)'},inplace=True)
...
if __name__ == '__main__':
    ...
    main_end_folder = 'c:/Users/1/PycharmProjects/Demetra/data/Результат ГИР ВУ'

    start_time = time.time()
    fix_files_girvu(main_data_folder, main_end_folder)
    end_time = time.time()
